...
\begin{tikzpicture}[
  ...
  \node[core, right=22mm of tool, yshift=9mm] (bel)
    {belief-derived desires\\\footnotesize\{ReachParcel, Deliver, Explore\}};
  \node[merge, below=7mm of bel] (queue) {Intention queue\\\footnotesize merge + sort (TTL)};
  \node[core, below=7mm of queue] (exec) {Planner $\rightarrow$ Executor};
  \draw[flow] (bel) -- (queue);
  \draw[flow] (queue) -- (exec);
  ...
  \node[peer, below=20mm of tool] (peeragents)
    {Teammate BDI agent(s)\\\footnotesize $\rightarrow$ their intention queue};

  % --- injections ---
  \draw[inject] (tool.east) to[out=0, in=180]
    node[above, font=\scriptsize, pos=0.55] {local inject} (queue.west);
