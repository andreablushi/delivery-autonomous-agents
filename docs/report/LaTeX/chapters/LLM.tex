...
Coordination in our system is fully decentralized and message-driven: there is no central authority, and every joint decision emerges from peer-to-peer exchanges layered on top of each agent's BDI loop.
Since only the LLM-driven agent is wired to the mission controller, it is the one that interprets admin queries and is therefore the sole agent able to issue proposals; purely reactive BDI teammates only evaluate the proposals they receive and vote on them.
