...
The tile is never made impassable: if no alternative path exists, the agent will still cross it.
Each penalty is keyed by an opaque identifier, enabling idempotent updates—re-registering the same identifier replaces the previous entry.
Like scoring rules and navigation goals, traversal penalties are propagated to all teammates upon registration.
...
All squad-level tool calls---navigation goals, scoring rules, traversal penalties, and traffic-control commands---follow a uniform two-step pattern: the injection is applied to the LLM agent locally first, then packaged as a peer-injection envelope and broadcast to all currently sensed teammates.
This ensures that a single LLM command propagates atomically across the entire squad without requiring the mission controller to address each agent individually.
The sole exception is \texttt{assign\_goto}, which is either a local self-injection or a point-to-point message to a named agent via \texttt{communicateTo}, never a broadcast.
...
Similarly, the \texttt{request\_red\_light} and \texttt{request\_green\_light} commands atomically inject or clear an indefinite \texttt{HOLD\_TILE} directive on the nearest odd-row tile, effectively halting or resuming team movement.
